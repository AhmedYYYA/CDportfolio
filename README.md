...
**v1.2** — glassmorphism design system (frosted cards, tables, modals and navigation over a drifting gold/blue aurora, with a solid-colour fallback and print-safe output) plus a lightweight interaction layer: KPI count-up, ripple feedback, hero parallax, below-the-fold reveals, scroll-aware header, and bilingual toasts — all dependency-free and disabled under reduced-motion. Includes a deep Arabic editorial revision and restores 18 shell strings (6 sidebar group headers and 12 role descriptions) that previously rendered empty in both languages; a permanent regression test now guards them.
**v1.1** — refined visual design system, disclaimer text in both languages, gate codes normalised to G0–G11, Arabic terminology corrections.
...
`35 / 35 PASS` — including: identical hash across independent loads; ledger identity
...
اجتاز الملف **35 من 35** فحصاً آلياً، من ضمنها مطابقة دفتر المخزون على 6,000 صف، وحفظ الحالات
